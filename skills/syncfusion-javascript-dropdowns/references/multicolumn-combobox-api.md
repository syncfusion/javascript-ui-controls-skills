# API Reference — Syncfusion JavaScript MultiColumnComboBox

Source: https://ej2.syncfusion.com/documentation/api/multicolumn-combobox/index-default
...
|---|---|---|---|
| `actionFailureTemplate` | `string \| Function` | `'Request failed'` | Template shown when a remote request fails |
...
| `footerTemplate` | `string \| Function` | `null` | Custom template for the popup footer |
...
| `groupTemplate` | `string \| Function` | `null` | Custom template for group headers |
...
| `itemTemplate` | `string \| Function` | `null` | Custom template for each popup row |
| `locale` | `string` | `''` | Locale code for localization (e.g., `'fr-BE'`) |
| `noRecordsTemplate` | `string \| Function` | `'No records found'` | Template shown when no data matches |
...
Call these on the instance returned by `new MultiColumnComboBox()`:
...
document.getElementById('getBtn')!.addEventListener('click', () =>
  console.log(mccBox.getDataByValue('1001'))
);
```
...
|---|---|---|
| `actionBegin` | `Object` | Fires before remote data fetch begins |
| `actionComplete` | `Object` | Fires after remote data fetch completes |
| `actionFailure` | `Object` | Fires when remote data fetch fails |
| `change` | `ChangeEventArgs` | Fires when the selected value changes |
| `close` | `PopupEventArgs` | Fires when the popup closes |
| `created` | `Event` | Fires after the component is first rendered |
| `filtering` | `FilteringEventArgs` | Fires on each filter input keystroke |
| `open` | `PopupEventArgs` | Fires when the popup opens |
| `select` | `SelectEventArgs` | Fires when a popup item is selected |

---

## ColumnModel

Defines the structure of each column in the popup. Used as elements of the `columns` array.
...
| `template` | `string \| Function` | — | Custom template for cell content |
| `headerTemplate` | `string \| Function` | — | Custom template for the header cell |
