# JavaScript Grid - Programmatic Control (Methods)
...
- Programmatically control grid behavior via methods
- Get and set grid state and configuration
- Trigger grid actions from external code
- Access grid data and selection programmatically
- Automate grid operations and workflows
...
Access all Grid methods through the Grid instance after creating or acquiring a reference. Methods enable programmatic control: data manipulation, selection, filtering, editing, export, and DOM navigation.
...
// Access methods: grid.addRecord(data)
```
...
**CRITICAL**: Follow these rules when using Grid's programmatic methods.

### Rule 1: Grid Instance Reference is MANDATORY for Method Calls

```ts
// ❌ WRONG - No instance reference
const grid: Grid = new Grid({ dataSource: data });

// ✅ CORRECT - Keep reference for method access
let gridInstance: Grid;
const grid: Grid = new Grid({ dataSource: data });
gridInstance = grid;
grid.appendTo('#grid');

// Access methods: gridInstance.addRecord(data)
```
...
grid.selectRows(0); // Number instead of array
...
### Rule 6: Edit Methods Require editSettings Configuration
...
### Basic Instance Creation and Method Access
...
// Manipulate data
gridInstance.addRecord({ OrderID: 99, CustomerID: 'CUST001', Freight: 25 });
gridInstance.deleteRecord('OrderID', selectedRecord);
gridInstance.updateRow(0, { Freight: 50 });
gridInstance.setCellValue('OrderID-99', 'Freight', 30);
...
const selectedIndexes = gridInstance.getSelectedRowIndexes();
...
```ts
// Auto-fit columns
gridInstance.autoFitColumns(['OrderID', 'Freight']);

// Show/hide columns
gridInstance.hideColumns(['CustomerID']);
gridInstance.showColumns(['CustomerID']);

// Reorder columns
gridInstance.reorderColumns('OrderID', 'Freight');

// Get column info
const columns = gridInstance.getColumns();
const columnIndex = gridInstance.getColumnIndexByField('Freight');
const column = gridInstance.getColumnByField('OrderID');
```
...
```ts
// Apply filter
gridInstance.filterByColumn('CustomerID', 'equal', 'VINET');

// Multi-column sort
gridInstance.sortColumn('OrderDate', 'Descending', true);
gridInstance.sortColumn('CustomerID', 'Ascending', true);

// Search
gridInstance.search('searchTerm');
...
```ts
// Requires editSettings configuration
const grid: Grid = new Grid({
  dataSource: data,
  columns: [...],
  editSettings: {
...
// Batch operations
gridInstance.editCell(0, 'Freight');
const changes = gridInstance.getBatchChanges();
gridInstance.batchUpdate(changes);
```
...
const handleExcelExport = async () => {
  await gridInstance.excelExport({
    fileName: 'GridData.xlsx',
    dataSource: gridInstance.getCurrentViewRecords()
  });
};
...
```ts
// Get DOM elements
const headerTable = gridInstance.getHeaderTable();
const contentTable = gridInstance.getContentTable();
const rows = gridInstance.getDataRows();
const cells = gridInstance.getCellFromIndex(0, 1);

// Get pager
const pager = gridInstance.getPager();
```
...
gridInstance.enableToolbarItems(['Add', 'Edit'], true);
gridInstance.enableToolbarItems(['Delete', 'Update'], false);

// Access toolbar module
if (gridInstance.toolbarModule) {
  gridInstance.toolbarModule.enableItems(['Search'], false);
}
```
...
if (gridInstance.detailRowModule) {
  gridInstance.detailRowModule.detailsExpand([0, 1, 2]);
  gridInstance.detailRowModule.detailsCollapse([0, 1, 2]);
}

// Or use direct Grid methods
gridInstance.detailExpandAll();
gridInstance.detailCollapseAll();
```
...
```ts
// Pin rows
const rowsToPin = gridInstance.getSelectedRecords();
gridInstance.pinRows(rowsToPin);

// Unpin rows
gridInstance.unpinRows(rowsToPin);
```
...
```ts
// Group by column
gridInstance.groupColumn('CustomerID');

// Expand/collapse groups
gridInstance.groupExpandAll();
gridInstance.groupCollapseAll();

// Remove grouping
gridInstance.ungroupColumn('CustomerID');
```
...
```ts
// Get persisted state
const persistData = gridInstance.getPersistData();
console.log(persistData);

// Enable persistence on Grid
const grid: Grid = new Grid({
  dataSource: data,
  enablePersistence: true,
  ej2StatePersistenceVersion: '1.0'
});
```
...
```ts
// Column Chooser
gridInstance.openColumnChooser(100, 100);

// Adaptive Dialogs
gridInstance.showAdaptiveFilterDialog();
gridInstance.showAdaptiveSortDialog();

// Loading Indicator
gridInstance.showSpinner();
setTimeout(() => gridInstance.hideSpinner(), 2000);
```
...
- **Grid Instance**: Always maintain a reference to the grid instance for method access. Store it in a variable or class property.
- **Module Injection**: Use `Inject()` to register required modules before calling related methods.
- **Promise Methods**: Export and some data methods return Promises - use `await` for completion.
- **Error Handling**: Wrap method calls in try-catch for production code, especially data operations.
- **Event Binding**: Use events like `actionBegin` and `actionComplete` to intercept operations before/after execution.
- **Performance**: For large datasets, use virtual scroll with `enableVirtualization` instead of loading all rows.
- **TypeScript**: Always use proper types (`Grid`, `Column[]`, etc.) for type safety and IDE support.
