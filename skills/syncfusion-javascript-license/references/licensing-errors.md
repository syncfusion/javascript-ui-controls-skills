...
{
  "dependencies": {
    "@syncfusion/ej2-grids": "~25.1.35",  // ✅ Only 25.1.x patches
...
License key generated for one platform but used in another (e.g., Angular key in JavaScript project).
...
# Expected for JavaScript: "@syncfusion/ej2-*" (without platform suffix)
```
...
```bash
npm install @syncfusion/ej2-grids@26.1.35 @syncfusion/ej2-buttons@26.1.35
```
...
```bash
# 1. Update packages
npm install @syncfusion/ej2-grids@26.1.35
...
```
Error: License key platform (Angular) does not match detected platform (JavaScript).
```

**Solution (v30.x.x):**
1. Generate JavaScript platform key
